...
export default function Window({
  ...
  if (local) {
    ...
    return (
      <StyledWindow
        key={id}
        className={`window ${live ? "live" : ""} ${active ? "active" : ""}`}
      >
        <h3
        ...
        >
          ...
          <div
            className="con"
            data-tab-count={`${tabs.length} ${chrome.i18n.getMessage("tabs")}`}
          >
            <input
          ...
          <Options>
            {live && (
              <li
                className="item"
                onClick={handleCopyLink.bind(null, winId, room)}
              >
            ...
            <li
              className="item"
              onClick={handleSaveWindow.bind(null, { id, winId })}
            >
        ...
        >
          {live ? (
            ...
            <span className="unsave">{chrome.i18n.getMessage("unsaved")}</span>
          )}
        </div>
      </StyledWindow>
    );
    ...
    return (
      <StyledWindow className={`window`}>
        <h3
          className={`title openable ${live ? "live" : ""}`}
        ...
        >
          ...
          <Options>
            ...
            <li
              className="item"
              onClick={handleRemoveWindow.bind(null, relation_id)}
            >
