...
export default function WindowList({
  ...
  return (
    <>
      {currentWindows && currentWindows.length !== 0 && (
        <StyledWrapper>
          <h2 className="title">
            {chrome.i18n.getMessage("current_window_title")}
          </h2>
          <div className={`block`}>
            {currentWindows
              .sort((a, b) => b.active - a.active)
              .map(({ active, title, id, winId, room, tabs, live }) => {
                return (
                  <Window
                    key={id}
                    data={{
      ...
      {savedWindows ? (
        <StyledWrapper>
          <h2 className="title">
            {chrome.i18n.getMessage("saved_window_title")}{" "}
            {loading && <MdOutlineRefresh className="tip" />}
          </h2>
          {windowLoading ? (
            <ContentLoader />
          ) : (
            <div className={`block ${savedWindows.length == 0 ? "empty" : ""}`}>
              {savedWindows.length == 0 && (
                <div className="tip">
                  You haven’t saved any windows yet. Start cobrowsing and save
                  any window that you would like to share again!
                </div>
              )}
              {savedWindows
                .sort((a, b) => {
                ...
                .map(
                  ({
                    ...
                    const localOpenedWindow = currentWindows.find(
                      (w) => w.winId == id
                    );
