...
const Content = {
  ...
  guest: (
    <>
      ...
      <ul>
        <li>Save the window to cobrowse later</li>
        <li>Start your own cobrowse session</li>
      </ul>
    </>
  ),
};
  ...
  return (
    <StyledModal>
      <div className="modal">
        ...
        <div className={`content ${modalType}`}>{Content[modalType]}</div>
        <div className="btns">
          <button onClick={handleQuit.bind(null, false)} className="btn ghost">
            {chrome.i18n.getMessage("quit")}
          </button>
          {modalType == "logined" ? (
            <button
              disabled={saving}
              onClick={handleQuit.bind(null, true)}
              className="btn"
            >
              {saving ? `Saving` : chrome.i18n.getMessage("save_quit")}
            </button>
          ) : (
