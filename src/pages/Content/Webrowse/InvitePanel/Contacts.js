...
const StyledWrapper = styled.section`
  width: 100%;
  .list{
    background: var(--webrowse-box-bg-color);
    ...
    .contact{
        ...
        .invite{
            ...
            padding:4px 9px;
            color:#FFFFFF;
            background-color: var(--webrowse-theme-color);
            border-radius: 5px;
            &.invited{
                border:1px solid var(--webrowse-theme-color);
...
const pushNotify = async (host, id, url = '') => {
  ...
  const resp = await fetch(PUSH_API, {
    ...
    body: JSON.stringify({
      to: [id],
      data: {
        message: `${host} invites you to webrowse!`,
        url
      },
      time_to_live: 2000
    })
...
export default function Contacts({ inviteUrl = '', username = "" }) {
  const { data, error, loading } = useSWR(
    username ? `//webrowse.nicegoodthings.com/members/authing/${username}` : null,
    fetcher
  );
