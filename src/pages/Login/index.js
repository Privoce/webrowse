...
const loginHandler = (user) => {
  console.log(user);
  // 本地种下用户数据
  const { id, username, nickname, email, photo } = user;
  const filteredUser = { id, username, nickname, email, photo };
  chrome.storage.sync.set({ user: filteredUser }, function () {
    console.log("user from login page ", user);
  });
  if (popup) {
    chrome.tabs.getCurrent((tab) => {
      console.log(tabId, filteredUser);
    ...
    chrome.tabs.getCurrent((tab) => {
      ...
      chrome.tabs.update(id, {
        url: `https://webrow.se/logined?name=${encodeURI(user.username)}`,
      });
    });
  }
};
guard.on("load", (authClient) => {
  ...
  const loginItems = [
    ...document.querySelectorAll(
      ".authing-social-login-list .authing-social-login-item"
    ),
  ];
