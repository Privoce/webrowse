...
const protocolPrefix =
  SOCKET_SERVER_DOMAIN.indexOf("localhost") > -1 ? "http://" : "wss://";
...
chrome.tabs.onActivated.addListener(({ tabId, windowId }) => {
  ...
  if (roomId || winId) {
    sendMessageToTab(tabId, { roomId, winId }, EVENTS.ROOM_WINDOW);
  }
});
// 安装扩展触发的事件
...
const sendTabSyncMsg = debounce((ws, socket) => {
  ...
  ws.read().then((wsUpdateCopy) => {
    ...
    socket.send({
      cmd: EVENTS.WORKSPACE,
      payload: { workspace: wsUpdateCopy },
    });
  });
}, 500);
...
const checkUpToTabLimit = (wid, tid) => {
  ...
  if (
    DATA_HUB.loginUser?.level == 0 &&
    DATA_HUB[wid].tabs.length >= TAB_LIMIT_COUNT
  ) {
...
const checkIsFollower = (wid, tid) => {
  ...
  const currentUser = DATA_HUB[wid].users?.find(
    (u) => u.id == DATA_HUB[wid].socketId
  );
...
const initWorkspace = async ({
  ...
  DATA_HUB[finalWindowId] = {
    ...
    fav: false,
    title: "",
    floaterTabVisible: { tab: true, follow: false, audio: false },
    tabs: [],
    createTabs: [],
    users: [],
    socketId: "",
  ...
  currWorkspace.addEventHandler(async ({ event, rawParams }) => {
    const currUser = DATA_HUB[finalWindowId].users.find(
      (u) => u.id == DATA_HUB[finalWindowId].socketId
    );
    // 捕捉并处理对应的tab事件
    let currSocket = null;
      ...
      case TabEvent.onActivated:
        // case TabEvent.onHighlighted:
        {
          ...
          if (
            ...
            // 标记鼠标tab
            CursorTabs[rawParams.activeInfo.windowId] =
              rawParams.activeInfo.tabId;
      ...
      case TabEvent.onUpdated:
        {
          ...
          console.log("tab update sync flag", {
            active,
            pageLoadingStatus,
            shouldSync,
            changeInfo,
          ...
          const isValidSyncTab =
            url && url.startsWith("http") && pageLoadingStatus == "complete";
          if (isValidSyncTab) {
            // 建立live connect, 用url当做name来日后识别
            Connections[`${windowId}_${url}`] = chrome.tabs.connect(tab.id, {
              name: url,
            });
            console.log(
              `page load status: ${pageLoadingStatus} `,
              changeInfo,
              tab
            );
            if (shouldSync) {
              ...
              // 从创建tab id 集合中移除
              DATA_HUB[windowId].createTabs = DATA_HUB[
                windowId
              ].createTabs.filter((tid) => tid !== tab.id);
            }
          }
        }
        break;
    ...
    notifyActiveTab({
      ...
      action: EVENTS.CHECK_CONNECTION,
    });
  });
};
// update tabs
...
onMessageFromPopup(MessageLocation.Background, {
  ...
  [EVENTS.POP_UP_DATA]: () => {
    ...
    sendMessageToPopup(
      { user: loginUser, windows: filteredWindows },
      MessageLocation.Background,
      EVENTS.POP_UP_DATA
    );
    sendMessageToPopup(
  ...
  [EVENTS.NEW_WINDOW]: ({
    ...
    let finalRoomId =
      roomId ||
      loginUser?.id ||
      `${Math.random().toString(36).substring(7)}_temp`;
    let finalWinId = isOpenedWindow
      ? `${Math.random().toString(36).substring(7)}_temp`
      : winId || `${Math.random().toString(36).substring(7)}_temp`;
    console.log({
      currentWindow,
      finalRoomId,
      finalWinId,
...
onMessageFromContentScript(MessageLocation.Background, {
  ...
  [EVENTS.ROOM_SOCKET_INIT]: (request = {}, sender) => {
    ...
    const socket = io(SOCKET_SERVER_URL, {
      ...
      query: {
        ...
        winId,
        temp,
        title: DATA_HUB.windowTitles[windowId] || "",
        invited: InvitedWindows[windowId],
        ...user,
      },
    });
    ...
    socket.on(EVENTS.TAB_EVENT, (operation) => {
      ...
      if (type == "remove") {
        ...
        notifyActiveTab({
          windowId,
          action: EVENTS.TAB_EVENT,
          payload: { username, type, tab },
        });
      }
    });
    // workspace 事件
  ...
  [EVENTS.ACCESS_TIP]: (req = {}, sender) => {
    ...
    if (currSocket) {
      currSocket.send({ cmd: EVENTS.ACCESS_TIP, payload: { site, index } });
    }
  },
  // host的鼠标move数据
  [EVENTS.HOST_CURSOR]: (req = {}, sender) => {
    ...
    if (currSocket) {
      currSocket.send({
        cmd: EVENTS.HOST_CURSOR,
        payload: { x, y, index, url },
      });
    }
  },
